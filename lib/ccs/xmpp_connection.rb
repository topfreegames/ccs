module CCS
  class XMPPConnection < XMPPSimple::Api
    ...
    def monitor_queue_ttl 
      CCS.debug "Renew queue #{id} ttl each #{CCS.configuration.default_queue_ttl_interval} seconds. If the queue is not on redis, drain! (ttl=#{CCS.configuration.queue_ttl_interval})"
